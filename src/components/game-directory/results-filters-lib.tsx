...
const filterConfigGame = [
  ...
  },
  {
    ...
    icon: (
      ...
      >
        <path
          d="M1.6665 14.168L9.99984 18.3346L18.3332 14.168M1.6665 10.0013L9.99984 14.168L18.3332 10.0013M9.99984 1.66797L1.6665 5.83464L9.99984 10.0013L18.3332 5.83464L9.99984 1.66797Z"
          stroke="#596184"
...
const ResultsFiltersLib = ({
  ...
  const handleOptionClick = async (option: any) => {
    console.log(
      " This is in handleOptionClick  handleOptionClick = >",
      option?.order
    );
    ...
    switch (option.by) {
      ...
      case "Total Hours":
        sort.by = "totalhours";
        setSelectedOptionDate({ ...sort, by: "Total Hours" });
        break;
      default:
        break;
    }
    dispatch(addFilter({ data: { filterType: "sort", filterKey: sort } }));
    dispatch(fetchGamesLibraryDataSuccess([]));
    filterdata.sort = filterdata?.sort?.map((item) => {
      return item.by === option.by ? { ...item, order: newOrder } : item;
    });
  };
        ...
        <div className="mb-2">
          ...
          >
            <div className="flex gap-3 items-center font-bold">
              ...
              Play Status
              {selectedFilters?.playStatus?.length
                ? ` (${selectedFilters?.playStatus?.length})`
                : ""}
            </div>
            <Image
          ...
          {isPlayStatusExpanded && (
            <div className="bg-[#15182B] p-4 rounded-2xl flex flex-col gap-4 mt-1">
              {filterConfigGame.map(({ label, key, icon }) => (
                <div className="checkbox" key={key}>
                  <label
                    className={`lib-custom custom-checkbox flex gap-2 font-bold text-sm text-[#596184] relative items-center cursor-pointer ${
        ...
        <div className="mb-2">
          ...
          >
            <div className="flex items-center gap-3 font-bold">
              ...
              />
              Game Progress{" "}
              {selectedFilters?.gameProgress?.length
                ? `(${selectedFilters.gameProgress?.length})`
                : ""}
            </div>
            <Image

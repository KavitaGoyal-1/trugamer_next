...
const LoginModalStatusBG = ({ isOpenLogin, onCloseLogin }: any) => {
  useEffect(() => {
    if (isOpenLogin) {
  ...
  return (
    <>
      <>
        ...
        <div
          className={`fixed pb-0 top-0 bottom-0 flex items-center justify-center left-0 w-full max-w-[950px] mx-auto right-0  h-full overflow-auto  rounded-2xl z-[99999999] transition-transform duration-500 ${
            isOpenLogin ? "translate-y-0" : "-translate-y-full"
          } z-40 custom-scrollbar`}
        >
          <div className=" m-8 w-full max-w-[90%] md:max-w-[480px] lg:max-w-[480px] 2xl:max-w-[500px] shadow-lg mx-auto rounded-2xl h-[90vh] 2xl:h-[85vh] flex items-center justify-center bg-[url('/home/statusBg.png')] bg-top  bg-cover bg-no-repeat">
            <div className="relative text-white py-4 md:py-8 px-4 md:px-12 rounded-xl shadow-lg bg-[#15182B]">
              {/* Close Button */}
              <button
...
export default LoginModalStatusBG;
